...
class TradingAgent:
    ...
    def _decide(self, context, assets):
        ...
        system_prompt = (
            ...
            "5. FUNDING RATE CONTEXT (Run 26+):\n"
            "   - Extreme positive funding (>0.03% daily): overleveraged longs, contrarian bearish, scale down long entries\n"
            "   - Extreme negative funding (<-0.03% daily): overleveraged shorts, contrarian bullish, favor longs\n"
            "   - Normal funding (-0.03% to +0.03%): market equilibrium, use quant signals as-is\n"
            "   - Use funding as a TILT for position sizing (reduce size into extreme funding, not a reversal signal)\n\n"
            "6. OVERRIDE only when you have strong evidence the quant signal is wrong:\n"
